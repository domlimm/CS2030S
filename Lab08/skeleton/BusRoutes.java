...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    String resultString = "Search for: " + this.stop + " <-> " + name + ":\n";
    resultString += "From " +  this.stop + "\n";

    CompletableFuture<String> result = CompletableFuture.completedFuture(resultString);
    
    for (BusService service: services.keySet()) {
        CompletableFuture<Set<BusStop>> stops = services.get(service);
        result = result.thenCombine(CompletableFuture.completedFuture(describeService(service, stops)),
   ...
   *     the bus services and matching stops served.
   */
  public String describeService(BusService service, CompletableFuture<Set<BusStop>> stops) {
    try {
        if (stops.get().isEmpty()) {
            return "";
        } 
        return stops.get().stream()
