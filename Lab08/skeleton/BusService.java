...
class BusService {
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    try {
        Scanner sc = new Scanner(BusAPI.getBusStopsServedBy(serviceId).get());
        CompletableFuture<Set<BusStop>> cf = CompletableFuture.supplyAsync(() -> {
  ...
  public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
    return CompletableFuture.supplyAsync(() ->  {
        try {
            return new HashSet<BusStop>(getBusStops().get()
                    .stream()
