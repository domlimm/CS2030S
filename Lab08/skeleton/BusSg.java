...
class BusSg {
  ...
  public static CompletableFuture<Set<BusService>> getBusServices(BusStop stop) {
      try {  
        Scanner sc = new Scanner(BusAPI.getBusServicesAt(stop.getStopId()).get());
        CompletableFuture<Set<BusService>> cf = CompletableFuture.supplyAsync(() -> {
  ...
  public static CompletableFuture<BusRoutes> findBusServicesBetween(BusStop stop, String searchString) {
    try {
      Map<BusService, CompletableFuture<Set<BusStop>>> validServices = getBusServices(stop)
          .get().stream()
